extends DialogScript

func _setup_dialog():
	dialog_lines = [
		{
			"speaker": "benny",
			"text": "Woah!",
			"audio": null,
			"animation": null
		},
		{
			"speaker": "flippy",
			"text": "Woah!",
			"audio": "talk1",
			"animation": null
		},
		{
			"speaker": "flippy",
			"text": "I didn't think anyone would be here yet",
			"audio": "talk2",
			"animation": null
		},
		{
			"speaker": "benny",
			"text": "What is happening right now?",
			"audio": null,
			"animation": null
		},
		{
			"speaker": "flippy",
			"text": "Something WONDERFUL!",
			"audio": "talk3",
			"animation": null
		},
		{
			"speaker": "flippy",
			"text": "You are going to be a Universal Celebrity after this",
			"audio": "talk1",
			"animation": null
		},
		{
			"speaker": "benny",
			"text": "I need to record this",
			"audio": null,
			"animation": null
		},
		{
			"speaker": "flippy",
			"text": "Well, more of a Universal Sports Icon I guess...",
			"audio": "talk2",
			"animation": null
		},
		{
			"speaker": "benny",
			"text": "Chat are you seeing this? is this real?",
			"audio": null,
			"animation": null
		},
		{
			"speaker": "flippy",
			"text": "I mean technically I'm a hologram but...",
			"audio": "talk3",
			"animation": null
		},
		{
			"speaker": "flippy",
			"text": "You've been selected to participate in a brand new game show",
			"audio": "talk1",
			"animation": null
		},
		{
			"speaker": "flippy",
			"text": "",
			"audio": "oh",
			"animation": null
		},
		{
			"speaker": "flippy",
			"text": "Sports League*",
			"audio": "talk2",
			"animation": null
		},
		{
			"speaker": "flippy",
			"text": "I gotta get better at remembering that",
			"audio": "talk3",
			"animation": null
		},
		{
			"speaker": "benny",
			"text": "What are you talking about? What is happening right now?",
			"audio": null,
			"animation": null
		},
		{
			"speaker": "flippy",
			"text": "Uhhhhh... TLDR you're gonna play a new type of Golf for our Universal Cable Network and if you win we're going to hook you up bigly",
			"audio": "talk1",
			"animation": null
		},
		{
			"speaker": "benny",
			"text": "a Golf show?",
			"audio": null,
			"animation": null
		},
		{
			"speaker": "flippy",
			"text": "You know what, this is always the most annoying part of all of this. How about you just click some buttons and see if you can figure it out",
			"audio": "talk2",
			"animation": null
		},
		{
			"speaker": "benny",
			"text": ".....",
			"audio": null,
			"animation": null
		}
	]
