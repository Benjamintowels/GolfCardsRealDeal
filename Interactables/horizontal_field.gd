...
# HorizontalField for generator puzzle system
# Spans between two Pylons on the same row and handles projectile collisions
...
func _update_ysort():
	"""Update the HorizontalField's z_index for proper Y-sorting"""
...
func setup_field(pylon1_ref: Node2D, pylon2_ref: Node2D):
	...
	if pylon1 and pylon2:
		# Calculate the width of the field based on pylon positions
		var field_width = abs(pylon2.global_position.x - pylon1.global_position.x)
		
		# Scale the field sprite to match the distance between pylons
		var sprite = get_node_or_null("FieldSprite")
		if sprite:
			sprite.scale.x = field_width / sprite.texture.get_width() if sprite.texture else 1.0
		...
		if collision_shape and collision_shape.shape:
			if collision_shape.shape is RectangleShape2D:
				collision_shape.shape.size.x = field_width
			elif collision_shape.shape is CircleShape2D:
				collision_shape.shape.radius = field_width / 2.0

func deactivate_field():
	"""Deactivate the force field when generator is powered down"""
	print("=== DEACTIVATING HORIZONTAL FIELD ===")
	...
	print("=== HORIZONTAL FIELD DEACTIVATED ===")

func get_collision_radius() -> float:
	"""
	Get the collision radius for this horizontal field.
...
func get_height() -> float:
	"""Get the height of this horizontal field for collision detection"""
	var height = Global.get_object_height_from_marker(self)
	print("Horizontal field height from marker:", height)
	return height

func _on_area_entered(area: Area2D):
	"""Handle collisions with the horizontal field area using proper height-based detection"""
	...
	if projectile and projectile.has_method("is_throwing_knife") and projectile.is_throwing_knife():
		...
		# The ball will call _handle_roof_bounce_collision on the horizontal field
		pass

func _on_area_exited(area: Area2D):
	"""Handle when projectile exits the horizontal field area - reset ground level"""
	var projectile = area.get_parent()
...
func _handle_horizontal_field_collision(projectile: Node2D) -> void:
	"""
	Handle collision with horizontal field - uses the roof bounce system for height-based collision detection.
	"""
	_handle_roof_bounce_collision(projectile)
...
func _handle_roof_bounce_collision(projectile: Node2D) -> void:
	...
	"""
	print("=== HORIZONTAL FIELD _handle_roof_bounce_collision CALLED ===")
...
func _reflect_off_field(projectile: Node2D) -> void:
	...
	# Calculate reflection direction (vertical reflection for horizontal field)
...
func _play_force_field_bounce_sound():
	...
	print("=== PLAYING HORIZONTAL FORCE FIELD BOUNCE SOUND ===")
	var bounce_sound = get_node_or_null("ForceFieldBounce")
	if bounce_sound and bounce_sound.stream:
		bounce_sound.play()
		print("Horizontal force field bounce sound played successfully")
	else:
		print("Horizontal force field bounce sound not found or no stream")
